...
app.get(`/user=:u_log_id`, async (req, res) => { //用户信息
  ...
  */
  try {
    ...
    let n_of_p = p_of_this_u.length
    let t1 = n_of_p + "";
    let str1 = '{"productCount":' + t1 + '}';
    let productCount = JSON.parse(str1); //数字转化JSON格式

    const watched = await prisma.$queryRaw `call pr_recent_watch(${u_info.id})`
    let n_of_w = watched.length
    let t2 = n_of_w + "";
    let str2 = '{"watchCount":' + t2 + '}';
    let watchCount = JSON.parse(str2); //数字转化JSON格式
    const stared = await prisma.$queryRaw `call pr_recent_star(${u_info.id})`
    const n_of_s = stared.length
    let t3 = n_of_s + "";
    let str3 = '{"starCount":' + t3 + '}';
    let starCount = JSON.parse(str3); //数字转化JSON格式

    const x = [u_info, productCount, watchCount, starCount]
...
app.get(`/userInfo/user=:u_log_id/product`, async (req, res) => {
  const {
    u_log_id
  } = req.params
  try {
    ...
    res.json(p_of_this_u_adj)
  } catch (error) {
    res.status(404).send('something wrong')
  }
})

app.get(`/userInfo/user=:u_log_id/watch`, async (req, res) => {
  const {
    u_log_id
  } = req.params
  try {
    ...
    res.json(watched_adj)
  } catch (error) {
    res.status(404).send('something wrong')
  }
})

app.get(`/userInfo/user=:u_log_id/star`, async (req, res) => {
  const {
    u_log_id
  } = req.params
  try {
...
app.post(`/user=:u_log_id/product=:p_name`, async (req, res) => { //关注、点赞、新建文件
  ...
  const {
    ...
    coworkers,
    f_name
  } = req.body //f_name只有在move=5时才应当被接收
  try {
